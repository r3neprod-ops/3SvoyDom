...
export default function LeadModal({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-[rgba(17,24,39,0.35)] p-4">
      <div className="w-full max-w-3xl rounded-2xl border bg-white p-5 shadow-hover md:p-6" style={{ borderColor: 'var(--border)' }}>
        <div className="mb-4 flex items-start justify-between gap-3">
          ...
          <button aria-label="Закрыть" className="text-lg text-[var(--muted)]" onClick={onClose} type="button">×</button>
        </div>

        <div className="mb-5 grid gap-4 md:grid-cols-[1fr_160px] md:items-start">
          <div>
            {!done && (
              <>
                ...
                <form onSubmit={submitStep} className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    {step > 0 && (
                      <Button onClick={() => setStep((prev) => prev - 1)} type="button" variant="secondary">
                        Назад
                      </Button>
                    )}
          ...
          <SlotBox
            className="order-first h-[160px] w-[140px] bg-[var(--surface2)] md:order-none md:h-[180px] md:w-[160px]"
            fileHint="mascot-modal.png"
            kind="mascot"
            slotKey="mascot-modal"
          />
        </div>
      </div>
    </div>
